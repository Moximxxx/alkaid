...
> **本项目技术栈**: Electron 33 + React 18 + TypeScript 5 + Vite 5 + Tailwind CSS 3 + LangChain + Vitest + bun

## 项目专属 CI/CD 流水线

### Builder 四阶段流水线（按顺序执行）

```
Phase 1: 类型检查 (typecheck) ──┐  失败→BLOCK
Phase 2: 代码检查 (lint)    ────┤  失败→WARN
Phase 3: 测试运行 (test)    ────┤  失败→BLOCK
Phase 4: 构建打包 (build)   ────┤  失败→BLOCK
```

| 阶段 | 命令 | 失败策略 | 说明 |
|:----|:----|:--------:|------|
| typecheck | `bun run typecheck` | BLOCK | TypeScript 类型检查（tsc --noEmit） |
| lint | `bun run lint` | WARN | ESLint 检查（0 errors 必须，warnings 不阻塞） |
| test | `bun run test` | BLOCK | Vitest 运行全部测试用例（140+ 测试） |
| build | `bun run build` | BLOCK | Vite 打包渲染进程 + tsc 编译 |
...
# 产出: dist/*.exe / dist/*.dmg / dist/*.AppImage
```

### GitHub Actions 参考工作流

项目当前为本地 Agent 驱动工作流，GitHub Actions 可作为未来补充。以下为参考配置：

```yaml
# .github/workflows/ci.yml — 参考配置（当前未启用）
name: CI
...
> **本项目适配**：BUILD + TEST + LINT（VERIFY 阶段即 deploy + verify 当前由 Agent 工作流手动执行）
